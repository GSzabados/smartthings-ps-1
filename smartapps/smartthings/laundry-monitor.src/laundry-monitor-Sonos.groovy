...
preferences {
	...
	section("Time thresholds (in minutes, optional)"){
        ...
        input "cycleCount", "decimal", title: "Number of cycles", required: false, defaultValue : 2
	}
}
...
{
	unsubscribe()
    log.debug "Monitor has been reset by user in app"
	initialize()
}
...
def accelerationActiveHandler(evt) {
	log.trace "vibration has been detected"
	if (!state.isRunning) {
...
def accelerationInactiveHandler(evt) {
	...
	if (state.isRunning) {
		...
		if (!state.stoppedAt) {
            ...
            log.debug "Waiting $delay and then will check to see if running / vibration"
		}
	}
}
...
def checkRunning() {
	...
	if (state.isRunning) {
		...
		if (!sensorStates.find{it.value == "active"}) {
			log.debug "Sensor is not active / vibrating"
			def cycleTimeMsec = cycleTime ? cycleTime * 60000 : 600000
			def duration = now() - state.startedAt
            
            // test to see if we can loop through cycleCount
            // loop through cycleCount
            // for each cycle, check if it is still running
            // if still running, reset and check again
            // else, move to next cycle
            // if end of cycleCount, end 
           if (state.cycleCount.toInteger() < cycleCount.toInteger()) {
           
           state.cycleCount = state.cycleCount.toInteger() + 1
           //might have to delay between each cycle?
           
           log.debug "Cycle $state.cycleCount is running"
           } else 
